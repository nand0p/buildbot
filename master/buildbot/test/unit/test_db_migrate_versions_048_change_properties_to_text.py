...
class Migration(migration.MigrateTestMixin, unittest.TestCase):
    ...
    def test_update(self):
        ...
        def verify_thd(conn):
            ...
            # Test column max limit
            change_properties = sautils.Table('change_properties', metadata, autoload=True)
            self.assertEqual(
                re.findall('\d+', str(sa.String(random_length))),
                re.findall('\d+', str(change_properties.c.property_value.type)))

            # Test write and read db random string
            ...
            q = conn.execute(sa.select([change_properties.c.property_value]))
            [self.assertEqual(q_string[0], unicode(random_string)) for q_string in q]
