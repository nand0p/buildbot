...
class Migration(migration.MigrateTestMixin, unittest.TestCase):
    ...
    def test_update(self):
        ...
        def verify_thd(conn):
            ...
            # Test insert $random_length byte string
            #insert into changes (changeid,author,comments,when_timestamp,repository,codebase,project) values (1,'a','b',1,'c','d','e');
            ...
            #property_value = sa.select([change_properties.c.property_value]).where(change_properties.c.changeid=1)
            #self.assertEqual(len(bytearray(conn.execute(property_value).fetchone())),random_length)
